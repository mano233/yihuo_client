<template>
  <div style="padding: 16px 16px 0 16px;background: white">
    ...
    <nut-button block @click="onClick" style="margin-top: 12px">点击更新</nut-button>
  </div>
</template>


<script>
import {getGoodsDetail,uploadImg,updateGoods} from '@/api/API'
...
export default {
  ...
  beforeCreate () {
    getGoodsDetail(this.$route.params.gid).then(e=>{
      this.$nextTick(()=>{
        new BScroll('#wrapper', {
          scrollX: true,
          click: true
      ...
      this.price = parseFloat(e.content.goods.price)
      this.cateLogId = e.content.goods.catelogId
      let contentJson = JSON.parse(e.content.goods.contnet)
      this.content = contentJson.text
      for (let i = 0;i<contentJson.imgs.length;i++) {
        this.imgList.push({
          id:i,
          src:contentJson.imgs[i]
        })
      }
    })
  },
    ...
    onClick () {
      ...
      if(this.beforeCheck()) {
        let data = {
          ...
          price: this.price,
          contnet: JSON.stringify({text:this.content,imgs}),
          preImg:imgs[0],
          title: this.title,
          catelogId:this.cateLogId,
        }
        updateGoods(data).then(e=>{
          if(e.code!==200){
            this.$dialog({
              content: "更新失败",
              noFooter:true,
          ...
          this.$router.replace('/goods/'+this.$route.params.gid)
        })
      }else{
        this.$dialog({
